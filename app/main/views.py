...
def load_applications(**kwargs):
    ...
    for app in apps:
        assets = ApplicationAssets.query.filter_by(app_id=app.id).first()
...
def load_applications1(**kwargs):
    ...
    for app in apps:
        ...
        print 'looped app assets =>', assets
        developer = Developer.query.filter_by(user_id=app.developer_id).first()
...
def index():
    ...
    for app_uuid in get_installed_apps():
        installed_apps.append(load_applications1(app_id=app_uuid))

    applications = load_applications1()
    for app in range(len(applications)):
        if applications[app]['uuid'] in get_installed_apps():
            applications.pop(app)
...
@main.route('/app_assets1/<string:app_uuid>', methods=['GET', 'POST'])
def app_assets1(app_uuid):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            flash('Assets added successfully', 'success')
        # return redirect(request.args.get('next') or url_for('main.app_info1', app_uuid=app_uuid))
        return redirect(request.args.get('next') or url_for('main.index'))
        # return redirect(request.args.get('next') or url_for('main.app_info', app_id=app_uuid))
...
@main.route('/app_info1/<string:app_uuid>')
def app_info1(app_uuid):
    application = Application.query.filter_by(uuid=app_uuid).first()
    # print 'Saved App =>', application.id
    app_id = application.id
    developer = Developer.query.filter_by(user_id=application.developer_id).first()
    assets = ApplicationAssets.query.filter_by(app_uuid=app_uuid).first()
    # print 'Saved App Assets =>', assets.icon

    app_details = load_applications1(app_id=app_uuid)
    print 'App Details =>', app_details
    related_apps = load_applications1(category_id=application.category_id, app_id=app_uuid)
...
def application():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('main.app_assets1', app_uuid=application.uuid))
    return render_template('application.html', form=form)
